...
Particle::Particle(SingleMesh& me, glm::vec3 p, int m)
{
	scale = rand() % 10 / 10.0f;
	health = 100;
...
{
	float constants = -0.5 * 1.225 * 0.09 * glm::pi<float>() * (p.scale) * (p.scale);
...
void ParticleSystem::init(int p, SingleMesh& m)
{
	...
	for (int i = -length / 2; i < length / 2; i++)
	{
		for (int j = -10; j < 10; j++)
		{
			...
			Particle particle(m, glm::vec3(0.0f, 20.0f, 0.0f), rand() % 400 + 100);
			if (particle.mass <= 0)
				particle.mass = 1;
